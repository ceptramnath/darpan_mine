...
import 'package:darpan_mine/Mails/Data%20entry%20daily%20report.dart';
...
class _UtilitiesMainScreen extends State<UtilitiesMainScreen> {
  ...
  void dayDialog(String type) async {
    ...
    // print(await BillerData().select().toMapList());
    ...
    //Added by RAkesh
    print("Last Day open check");
    var test1 =
    '''select MAX(substr(DayBeginDate,7,4)|| '-'||substr(DayBeginDate,4,2)||'-' ||substr(DayBeginDate,1,2)) as LastOpenedDate from DayModel''';
    final lastDayOpen = await BookingModel().execDataTable(test1);
    print(lastDayOpen.toString());
    print(lastDayOpen.length.toString());
    print(lastDayOpen[0]["LastOpenedDate"]);

    if(lastDayClose[0]["LastClosedDate"] == lastDayOpen[0]["LastOpenedDate"])
      print("last Open date = Last closed date ");
    else
      print("last Open date not equal Last closed date ");
    //Addition ended
    ...
    // Checking whether BODA generated for the previous Day
    final lastday = lastDayOpen[0]["LastOpenedDate"].toString()
        .split('-')
        .reversed
        .join('-').toString();
    final bodaData = await BodaBrief()
        .select()
        .BodaGeneratedDate
        .equals(lastday)
        .toCount();
    ...
    if (type == 'Begin') {
                          ...
                          TextButton(
                              onPressed: () async {
                                ...
                                if (dayData.isEmpty) {
                                  ...
                                  // if(lastDayClose[0]["LastClosedDate"].toString().isEmpty &&
                                  //   lastDayOpen[0]["LastOpenedDate"].toString().isNotEmpty)
                                if(lastDayClose[0]["LastClosedDate"] != lastDayOpen[0]["LastOpenedDate"])
                                {
                                  dayMessageDialog('Close the Day for the ${lastDayOpen[0]["LastOpenedDate"]}');
                                 setState(() {
                                    dayBegin = false;
                                  });
                                  }
                                ...
                                //Checking whether BODA generated for the previous Day
                                  ...
                                  dayMessageDialog('BODA is not generated for the day $lastday . Please generate.');
                                  setState(() {
                                    dayBegin = false;
                                  });
                                }
                              ...
                              child: Text("Yes")):TextButton(child: Text("Yes"), onPressed: (){},)
                        ],
                      ),
                      _isNewLoading == true
                          ? Loader(
      ...
      // else {
      //   print("Day Begin for $currentDate");
      //   print(double.parse(lastClosingBalance).toStringAsFixed(2));
      //   await BookingDBService().updateDay('Begin', currentDate, currentTime,
      //       double.parse(lastClosingBalance).toStringAsFixed(2)
      //       //walletAmount.toString()
      //       //
      //       );
      ...
      //
      //   //before deletion insert into Backup Table available in Report DB.
      //   final fetchCashTable =await CashTable().select().toList();
      //   if(fetchCashTable.length>0) {
      //     for (int i = 0; i < fetchCashTable.length; i++) {
      //       final a = await CashTableBackup()
      //         ..Sl = fetchCashTable[i].Sl
      ...
      //         ..Cash_Date = fetchCashTable[i].Cash_Date
      //         ..Cash_Time = fetchCashTable[i].Cash_Time
      //         ..Cash_Description = fetchCashTable[i].Cash_Description
      //         ..Cash_Type = fetchCashTable[i].Cash_Type;
      //       await a.save();
      //     }
      //   }
      //
      //   final fetchTransTable =await TransactionTable().select().toList();
      //
      //   if(fetchTransTable.length>0) {
      //     for (int i = 0; i < fetchTransTable.length; i++) {
      //       final b = await TransactionTableBackup()
      ...
      //         ..tranid = fetchTransTable[i].tranid
      //         ..tranType = fetchTransTable[i].tranType
      //         ..tranDescription = fetchTransTable[i].tranDescription
      //         ..tranDate = fetchTransTable[i].tranDate
      //         ..tranTime = fetchTransTable[i].tranTime
      //         ..tranAmount = fetchTransTable[i].tranAmount
      //         ..valuation = fetchTransTable[i].valuation;
      ...
      //   if (cashCount == 0) {
      //     final addCash = CashTable()
      //       ..Cash_ID = 'OB_${DateTimeDetails().dateCharacter()}'
      //       ..Cash_Date = DateTimeDetails().currentDate()
      //       ..Cash_Time = DateTimeDetails().onlyTime()
      //       ..Cash_Type = 'Add'
      //       ..Cash_Amount = double.parse(
      ...
      //   } else {
      //     await LogCat()
      //         .writeContent('Cash Table already having $cashCount records.');
      //   }
      //
      //   int transCount = await TransactionTable().select().toCount();
      //   if (transCount == 0) {
      //     final addTransaction = TransactionTable()
      //       ..tranid = 'OB_${DateTimeDetails().dateCharacter()}'
      //       ..tranDescription = "Previous Day CB"
      ...
      final vpselect =await VPPEMO().select()
          ...
          .and
          .DATE_OF_DELIVERY
          .equals(DateTimeDetails().onlyDate()).toList() ;
      ...
      if (cbstrans.length > 0) {
      ...
      }else if (vpselect.length > 0) {
        Toast.showFloatingToast(
            "Please Book VPMO for the VP Delivery!",
            context);
      } else {
        if (dayData.isEmpty) {
        ...
        else if (lastDayClose[0]["LastClosedDate"] != lastDayOpen[0]["LastOpenedDate"]) {
          print('Condition ---1');
          showDialog(
              context: context,
              builder: (BuildContext context) {
                return AlertDialog(
                  content: Text('Do you wish to do \nDAY END for date: ${lastDayOpen[0]["LastOpenedDate"]}'),
                  actions: [
                    TextButton(
                    ...
                    TextButton(
                        onPressed: () async {
                          dayMessageDialog(
                              'Day End of ${lastDayOpen[0]["LastOpenedDate"]} happened successfully at $currentTime');
                          final lastdate = lastDayOpen[0]["LastOpenedDate"].toString()
                              .split('-')
                              .reversed
                              .join('-').toString();
                          print("Last daybegin date is $lastdate");
