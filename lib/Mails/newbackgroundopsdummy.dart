...
//       //var officeId='1234';
//       var officeId =
//           'BO52104101001'; //'BO20105103005';//'BO20105103005';//'BO20105103004';//'BO20105103005';
...
//               'MO Commission Master-VPP', DateTimeDetails().currentDateTime());
//         }
//
//         ///Biller Data
//
...
//           }
//           await addFileToDB(entities[i].toString(), dir.toString(),
//               'MO Commission Master-INS', DateTimeDetails().currentDateTime());
//         }
//
//         ///EMO MO Commission
...
//           }
//           await addFileToDB(entities[i].toString(), dir.toString(),
//               'MO Commission Master-EMO', DateTimeDetails().currentDateTime());
//         }
//
//         ///Products Master
...
//               subRes
//                   .add(additionalServiceMappingResult[i].toString().split("‡"));
//             }
//             for (int i = 0; i < subRes.length - 1; i++) {
//               if (subRes[i]
//                   .toString()
...
//             }
//           }
//           await addFileToDB(entities[i].toString(), dir.toString(), 'Switch On',
//               DateTimeDetails().currentDateTime());
//         }
//
//         ///Service master
...
//       //   }
//       //
//       //   if(File('storage/emulated/0/Darpan_Mine/DownloadCSV/$type-$officeId.zip').existsSync()){
//       //     var headers = {
//       //       'filename': '$officeId',
//       //       'Authorization': 'Bearer ${acctoken[0].AccessToken}',
//       //     };
...
//       //       await File("storage/emulated/0/Darpan_Mine/DownloadCSV/$fname-$officeId.zip")
//       //           .rename(
...
//           " A.ART_NUMBER = B.ART_NUMBER AND  DATETIME(A.REMARK_DATE,A.DELIVERY_TIME) < DATETIME ('now','-30 minutes','localtime') AND IS_COMMUNICATED IS NULL AND FILE_NAME IS NULL AND ART_STATUS = 'D' AND " +
//           " UPPER(MATNR) NOT IN (SELECT DISTINCT UPPER(ARTICLECODE) FROM ARTICLEMASTER WHERE UPPER(ARTICLETYPE)='EMO') ";
//
...
//           " DATETIME(A.REMARK_DATE,A.DELIVERY_TIME) < DATETIME ('now','-30 minutes','localtime') AND IS_COMMUNICATED IS NULL AND FILE_NAME IS NULL AND ART_STATUS IN ('N','O') " +
...
//
//       final artdeliveryexec = await DeliveryModel().execDataTable(artDelivered);
//       final artundeliveryexec =
//       await DeliveryModel().execDataTable(artUndelivered);
//       final emodeliveryexec = await DeliveryModel().execDataTable(emoDelivered);
...
//         row.add(emodeldata[i]['R_MONEY_DELIVERED']);
...
//       final bagreceivedexec = await DeliveryModel().execDataTable(bagreceived);
...
//       final Bagdispatch_File = await DeliveryModel().execDataTable(bagdispatch);
...
//         final dataDir = Directory("storage/emulated/0/Darpan_Mine/Uploads/Delivery");
//         zipFile = File(
...
//         final dataDir = Directory("storage/emulated/0/Darpan_Mine/Uploads/Booking");
...
//
//         if (File('storage/emulated/0/Darpan_Mine/UploadCSV/$file').existsSync()) {
...
//           // var request = http.MultipartRequest('POST', Uri.parse('https://gateway.cept.gov.in/sigfilecsi/upload'));
//
...
//
//             await LogCat().writeContent(
//                 "${DateTimeDetails().currentDateTime()}: File: $file: $res ");
