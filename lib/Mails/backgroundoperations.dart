...
class DataSync {
  ...
  bool JwtCheck(List<USERLOGINDETAILS> loginDetails) {
    print('JWT Checking in Background Op..!');
    bool rtValue = false;
  ...
  Future<void> download() async {
    ...
    if (ofcMaster.length > 0) {
      ...
      //Try for Delivery
        ...
        print("Entered else for delivery");
        try {
          await LogCat().writeContent(
              "${DateTimeDetails().currentDateTime()}: Delivery File Download");
          Response response = await dio.get( APIEndPoints().fileDownload+"?filename=$officeId&type=DELAPPS",
      ...
      //Try for Booking
        ...
        try {
          Response response = await dio.get(
            ...
            APIEndPoints().fileDownload+"?filename=$officeId&type=DEMOBOOKING",
	...
	///PaySlip
          ...
          if (entities[i].toString().contains('Payslip')) {
            print("Inside payslip code");
            print(entities[i]);
            print(i);

            subRes.clear();
            print("Inside payslip data");
            final PayslipFile = await File(entities[i].path).openRead();
            final PayslipFiles = await PayslipFile
                .transform(utf8.decoder)
                .transform(const CsvToListConverter())
                .toList();
            for (int i = 0; i < PayslipFiles.length; i++) {
              List PayslipResult = PayslipFiles
                  .toString()
                  .replaceAll("[", "")
                  .replaceAll("]", "")
                  .split("\n");
              for (int i = 1; i < PayslipResult.length; i++) {
                subRes.add(PayslipResult[i].toString().split("‡"));
              }
              print("Pay Slip Data file");
              ...
              for (int i = 0; i < subRes.length; i++) {
                if (subRes[i]
                    ...
                    .isNotEmpty &&
                    subRes[i][0].toString() != "") {
                  final addPaySlip = PaySlipData()
                    ..EMPLOYEE_ID = subRes[i][0]
                    ..EMPLOYEE_NAME= subRes[i][1]
                    ..OFFICE = subRes[i][2]
                    ..POSITION = subRes[i][3]
                    ..MONTH = subRes[i][4]
                    ..YEAR = subRes[i][5]
                    ..ACCOUNT_NUMBER = subRes[i][6]
                    ..TRCA = subRes[i][7]
                    ..DEARNESS_ALLOWANCE = subRes[i][8]
                    ..FIXED_STATIONERY_CHARGES = subRes[i][9]
                    ..BOAT_ALLOWANCE = subRes[i][10]
                    ..CYCLE_MAINTENANCE_ALLOWANCE = subRes[i][11]
                    ..OFFICE_MAINTENANCE_ALLOWANCE = subRes[i][12]
                    ..CDA = subRes[i][13]
                    ..COMBINATION_DELIVERY_ALLOWANCE = subRes[i][14]
                    ..COMPENSATION_MAIL_CARRIER = subRes[i][15]
                    ..RETAINERSHIP_ALLOWANCE = subRes[i][16]
                    ..CASH_CONVEYANCE_ALLOWANCE = subRes[i][17]
                    ..PERSONAL_ALLOWANCE_GDS = subRes[i][18]
                    ..BONUS_GDS = subRes[i][19]
                    ..EXGRATIA_GRATUITY = subRes[i][20]
                    ..SEVERANCE_AMOUNT = subRes[i][21]
                    ..INCENTIVES = subRes[i][22]
                    ..DA_ARREARS_GDS = subRes[i][23]
                    ..TOTAL_EARNINGS = subRes[i][24]
                    ..TRCA_ALLOWANCE_ARREARS = subRes[i][25]
                    ..DEARNESS_ALLOWANCE_ARREAR = subRes[i][26]
                    ..DEARNESS_RELIEF_CDAARREAR = subRes[i][27]
                    ..TOTAL_ARREARS = subRes[i][28]
                    ..COURT_ATTACHMENT_ODFM = subRes[i][29]
                    ..RD = subRes[i][30]
                    ..COURT_ATTACHMENT_DFM = subRes[i][31]
                    ..LICENSE_FEE = subRes[i][32]
                    ..SDBS = subRes[i][33]
                    ..AUDIT_OFFICE_RECOVERY = subRes[i][34]
                    ..COOP_CREDIT_SOCIETY = subRes[i][35]
                    ..RELIEF_FUND = subRes[i][36]
                    ..DEATH_RELIEF_FUND = subRes[i][37]
                    ..WATER_TAX = subRes[i][38]
                    ..ELECTRICITY_CHARGES = subRes[i][39]
                    ..AOR_NONTAX = subRes[i][40]
                    ..POSTAL_RELIEF_FUND = subRes[i][41]
                    ..PLI_PREMIUM = subRes[i][42]
                    ..RECREATION_CLUB = subRes[i][43]
                    ..UNION_ASSOCIATION = subRes[i][44]
                    ..WELFARE_FUND = subRes[i][45]
                    ..CGEGIS = subRes[i][46]
                    ..CGHS = subRes[i][47]
                    ..EDAGIS_92 = subRes[i][48]
                    ..LIC_PREMIUM = subRes[i][49]
                    ..CGIS = subRes[i][50]
                    ..RPLI = subRes[i][51]
                    ..PLI_SERVICE_TAX = subRes[i][52]
                    ..CONNECTIONS = subRes[i][53]
                    ..CGEWCC = subRes[i][54]
                    ..SOCIETIES = subRes[i][55]
                    ..FNPO = subRes[i][56]
                    ..ED_GIS = subRes[i][57]
                    ..SECURITY_BONDS = subRes[i][58]
                    ..EXTRA_DEPARTMENTAL_UNIONS = subRes[i][59]
                    ..CGEGIS_INSURANCE_FUND = subRes[i][60]
                    ..POSTAL_COOP_SOCIETY = subRes[i][61]
                    ..COOP_BANK_REC = subRes[i][62]
                    ..DIVISION_SPORTS_BOARD = subRes[i][63]
                    ..MISC_DEDUCTIONS = subRes[i][64]
                    ..BONDS = subRes[i][65]
                    ..CWFGDS = subRes[i][66]
                    ..FESTIVAL_ADVANCE_SPECIAL = subRes[i][67]
                    ..TOTAL_DEDUCTIONS = subRes[i][68]
                    ..TOTAL_GROSS_AMOUNT = subRes[i][69]
                    ..NET_PAY = subRes[i][70];

                    await addPaySlip.save();
                }
              }
              final b = await PaySlipData().select().toMapList();
            }
            // addFileToDB(entities[i].toString(), dir.toString(), 'Biller Data', DateTimeDetails().currentDateTime());
          }																	   
          ///Ins MO Commission
          ...
          try {
            ...
            LogCat().writeContent("Error in Prodessing Product Master File");
            LogCat().writeContent(e.toString());
          ...
          // print("Payslip insert");
          // if (entities[i].toString().contains('Payslip')) {
          //   print("Payslip inside");
          //   subRes.clear();
          //   final paySlip = await File(entities[i].path).openRead();
          //   final payslipFields = await paySlip
          //       .transform(utf8.decoder)
          //       .transform(const CsvToListConverter())
          //       .toList();
          //   print(payslipFields);
          //   for (int i = 0; i < payslipFields.length; i++) {
          //     List paySlipResult = payslipFields[i]
          //         .toString()
          //         .replaceAll("[", "")
          //         .replaceAll("]", "")
          //         .split("\n");
          //     print(paySlipResult);
          //     for (int i = 0; i < paySlipResult.length; i++) {
          //       subRes.add(paySlipResult[i].toString().split("‡"));
          //     }
          //     print(subRes);
        ...
        for (int i = 0; i < 2; i++) {
          ...
          if (File('storage/emulated/0/Darpan_Mine/DownloadCSV/$fname-$officeId.zip')
            ...
            var request = http.Request('GET', Uri.parse(
              APIEndPoints().fileAck+"?filename=$officeId&type=$type"));
  ...
  Future<void> upload() async {
    ...
    emoDeliveredrows.add(row);
        ...
        print("Bag art Data length: ${bagartdata.length}");
        try {
    ...
    print("Completed try block");
        ...
        if (emoBooking.isNotEmpty) {
          ...
          String csv =
              const ListToCsvConverter(fieldDelimiter: "‡",eol: "\r\n").convert(rows) +
                  "\r\n";//Trying EMO Bkg file with CRLF in middle and end also
          // String csv =
          //     const ListToCsvConverter(fieldDelimiter: "‡").convert(rows) +
          //         "\n";
           ...
           // await USERLOGINDETAILS().select().toList();
            ...
            if (File('storage/emulated/0/Darpan_Mine/UploadCSV/$file')
              ...
              var request = http.MultipartRequest('POST',
                  Uri.parse(APIEndPoints().fileUpload));
              ...
              if (response.statusCode == 200) {
                ...
                //After uploading the file successfully update the status to 1
                await FILE_SYNC_DETAILS()
                    .select()
                    .ZIP_FILE_NAME
                    .equals(file)
