...
public class UsbPrintersConnections extends UsbConnections {
    ...
    public static UsbConnection selectPrinter(String deviceId,Context context) {
       UsbConnection selectedUSBPrinter = null;
        UsbPrintersConnections printers = new UsbPrintersConnections(context);
        UsbConnection[] bluetoothPrinters = printers.getList();
        for(int i=0;i<bluetoothPrinters.length;i++){

            if(bluetoothPrinters[i].getDevice().getDeviceName()==deviceId)

            selectedUSBPrinter= bluetoothPrinters[i];
        }
